...
import { Droplet, Trophy, Shield } from 'lucide-react';
  ...
  useEffect(() => {
    const loop = (t: number) => {
      ...
      for (let i=0; i<tr.length; i++) {
        ...
        if (u.state === 'attacking' && u.targetId) {
          ...
          if (!tar || Math.hypot(tar.x-u.x, tar.y-u.y) > (u.template.range||1)*20*1.5) { u.state = 'idle'; }
            ...
            const windUp = 0.5; // wait 0.5s before hitting
            u.lastAttack += dt;
      ...
      tw.forEach(tower => {
         ...
         if (sp.some(s => s.type === 'freeze' && s.side!==tower.side && Math.hypot(s.x-tower.x, s.y-tower.y)<=s.radius*20)) return;
